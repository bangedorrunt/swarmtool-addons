...
describe("Memory Lane System", () => {
  ...
  beforeEach(async () => {
    ...
    currentTestDir = path.join(process.cwd(), `.hive-test-lane-${crypto.randomUUID()}`);
    ...
    const baseAdapter = {
      store: mock(async (args) => ({ id: `mem-${crypto.randomUUID()}`, message: "Stored" })),
      find: mock(async (args) => ({ results: [], count: 0 })),
      get: mock(async (args) => null),
      remove: mock(async (args) => ({ success: true })),
      validate: mock(async (args) => ({ success: true })),
      list: mock(async (args) => []),
      stats: mock(async () => ({ memories: 0, embeddings: 0 })),
      checkHealth: mock(async () => ({ ollama: true })),
      upsert: mock(async (args) => ({ operation: "ADD", reason: "new", memoryId: "new-id" }))
  ...
  test("should record feedback and adjust ranking", async () => {
    ...
    const itemX = {
      ...
      score: 0.8,
      collection: "memory-lane",
      metadata: JSON.stringify({ lane_version: "1.0.0", memory_type: "learning", feedback_score: 1.0 }),
      createdAt: new Date().toISOString()
    };
    const itemY = {
      id: "id-y",
