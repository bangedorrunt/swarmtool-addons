...
export function createActorTools() {
  return {
    ...
    actor_init: tool({
      ...
      args: {
        goals: tool.schema
          ...
          .describe('List of goals for this orchestration'),
    ...
    actor_phase: tool({
      description:
        'Transition the actor to a new workflow phase. Use after completing current phase.',
      args: {
        phase: tool.schema
    ...
    actor_assumption: tool({
      ...
      args: {
        ...
        confidence: tool.schema.number().min(0).max(1).describe('Confidence level (0-1)'),
    ...
    actor_status: tool({
      description:
        'Get the current actor state including phase, goals, assumptions, and sub-agents.',
      args: {},
      async execute() {
        const projectPath = process.cwd();
        const state = await loadActorState(projectPath);
        ...
        const completedSubAgents = Object.values(state.subAgents).filter(
    ...
    actor_direction: tool({
      ...
      args: {
        ...
        decisions: tool.schema
          .array(tool.schema.string())
          .optional()
          .describe('New decisions to record'),
      },
      async execute(args) {
        const projectPath = process.cwd();
        const state = await loadActorState(projectPath);
