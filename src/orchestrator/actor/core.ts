...
function buildLedgerContent(state: ActorState): string {
    ...
    sections.push('# LEDGER.md');
    sections.push('');
    sections.push(`> Auto-generated by Chief-of-Staff Actor. Last updated: ${state.lastUpdated}`);
    sections.push('');
    ...
    // Event Offset (for resumption)
    sections.push('## Resumption');
    sections.push(`Event offset: ${state.eventOffset}`);
    sections.push('');
...
function eventToMessage(event: any): ActorMessage | null {
    ...
    switch (type) {
        ...
        case 'assumption.track':
            return { type, payload: { assumed: event.assumed, confidence: event.confidence, worker: event.worker } };
        case 'subagent.spawn':
            return { type, payload: { agent: event.agent, sessionId: event.session_id, prompt: event.prompt } };
        case 'subagent.complete':
            return { type, payload: { sessionId: event.session_id, agent: event.agent, result: event.result } };
        case 'subagent.failed':
            return { type, payload: { sessionId: event.session_id, agent: event.agent, error: event.error } };
        default:
            return null;
    }
}
