...
// 3. Available skills to inject into workers
use skill
```
...
**Solution:** Query Memory Lane FIRST.
...
const relevantLearnings = await memory_lane_find({
...
await memory_lane_store({
  information:
    'OAuth refresh tokens need 5min buffer to avoid race conditions. Without buffer, token refresh can fail mid-request if expiry happens between check and use.',
  type: 'insight',
