...
const loadSkill = () => {
  ...
  const skillPath = path.join(
    process.cwd(),
    'src/orchestrator/chief-of-staff/agents/oracle/SKILL.md'
  );
...
evalite('Oracle Agent Strategy', {
  ...
  task: async (input: any) => {
    ...
    if (
      typeof input === 'string' &&
      (input.toLowerCase().includes('refactor') || input.toLowerCase().includes('schema'))
    ) {
      return {
      ...
      return {
        epic: { title: 'Feature', request: String(input) },
