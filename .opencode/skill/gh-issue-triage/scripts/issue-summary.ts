...
async function getIssueSummary(repo: string, issueNumber: number): Promise<Issue> {
  ...
  const result =
    await Bun.$`gh issue view ${issueNumber} --repo ${repo} --json number,title,body,state,author,labels,url,createdAt,updatedAt`.json();
...
function formatForCell(
  issue: Issue,
  contributorInfo?: { name?: string | null; twitter?: string | null }
): string {
  const lines = [
    issue.url,
    '',
