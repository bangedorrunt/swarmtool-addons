...
describe('extractSeverity', () => {
  ...
  test('detects suggestion severity', () => {
    expect(extractSeverity('💡 **Suggestion**: Consider using')).toBe('suggestion');
  });
...
\`\`\``;
  ...
  test('returns false for no code block', () => {
    expect(hasProposedFix('💡 **Suggestion**: Just do it differently')).toBe(false);
  });
});
...
describe('triage', () => {
  ...
  test('marks bot root comments as needing body', () => {
    const result = triage(mockComments);
    const botRoot = result.find((c) => c.author === 'coderabbitai' && c.inReplyToId === null);
  ...
  test('marks bot replies as not needing body', () => {
    const result = triage(mockComments);
    const botReply = result.find((c) => c.author === 'coderabbitai' && c.inReplyToId !== null);
