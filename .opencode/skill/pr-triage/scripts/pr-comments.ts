...
async function gh<T>(args: string[], schema?: z.ZodSchema<T>): Promise<T | string> {
 ...
 */
export async function fetchMetadata(repo: string, pr: number): Promise<CommentMetadata[]> {
 ...
 */
export async function fetchBody(repo: string, commentId: number): Promise<CommentFull> {
  const { owner, name } = parseRepo(repo);
 ...
 */
export async function summarizeByFile(repo: string, pr: number): Promise<FileSummary[]> {
 ...
 */
export function refineTriage(comment: TriagedComment, body: string): TriagedComment {
  ...
  switch (command) {
    ...
    case 'reply': {
      ...
      if (!repo || !pr || !commentId || !body) {
        console.error('Usage: pr-comments.ts reply <owner/repo> <pr> <comment_id> "<body>"');
        process.exit(1);
      }
    ...
    case 'unreplied': {
      ...
      const unreplied = comments.filter((c) => c.inReplyToId === null && !repliedTo.has(c.id));
